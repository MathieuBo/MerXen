"""Legacy cross-section registration using Spateo and expression features."""
...
from merxen.config import AlignmentConfig, LegacySpateoAlignmentConfig
...
SPATEO_INSTALL_MESSAGE = (
    ...
    'For direct CLI use, run: pip install -e ".[legacy-alignment]" '
    '&& pip install "anndata>=0.12.10"'
)

SpateoRunner = Callable[
    [ad.AnnData, ad.AnnData, LegacySpateoAlignmentConfig],
...
def run_spateo_alignment(
    fixed: ad.AnnData,
    moving: ad.AnnData,
    config: LegacySpateoAlignmentConfig,
) -> tuple[ad.AnnData, ad.AnnData]:
...
def _candidate_spateo_configs(
    config: LegacySpateoAlignmentConfig,
) -> list[LegacySpateoAlignmentConfig]:
    if not config.tune:
        return [config]
    ...
    return [
        LegacySpateoAlignmentConfig.model_validate(base | params) for params in grid
    ]
...
def _score_aligned_candidate(
    ...
    *,
    ...
    from merxen.alignment.legacy_qc import compute_grid_alignment_metrics
...
def _candidate_param_delta(
    base: LegacySpateoAlignmentConfig,
    candidate: LegacySpateoAlignmentConfig,
) -> dict[str, Any]:
